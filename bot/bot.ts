        ...
        if (groups) {
            for (const group of groups) {
                ...
                for (let i = 0; i < finalPostCount; i++) {
                    const post = posts.nth(i);\n                    const postText = (await post.innerText()).trim();\n\n                    // --- Attempt to extract real Facebook Post ID ---\n                    let postID = null;\n\n                    // Try finding permalink from timestamp/date link\n                    const timestampLink = post.locator(\'a[href*=\"/posts/\"], a[href*=\"story_fbid=\"], a[aria-label*=\"Time\"], a[aria-label*=\"Date\"]\').first();\n                    const permalink = await timestampLink.getAttribute(\'href\').catch(() => null);\n                    \n                    if (permalink) {\n                        const matchFbid = permalink.match(/story_fbid=(\\d+)/);\n                        if (matchFbid && matchFbid[1]) {\n                            postID = matchFbid[1];\n                            console.log(`    Extracted Post ID (story_fbid): ${postID}`);\n                        }\n                        if (!postID) {\n                            const matchPostPath = permalink.match(/\/posts\\/(\\d+)/);\n                            if (matchPostPath && matchPostPath[1]) {\n                                postID = matchPostPath[1];\n                                console.log(`    Extracted Post ID (from /posts/): ${postID}`);\n                            }\n                        }\n                    }\n                    \n                    // Fallback to data-ft attribute on a parent element\n                    if (!postID) {\n                        const dataFtElement = await post.locator(\'[data-ft]\').first().elementHandle();\n                        const dataFt = await dataFtElement?.getAttribute(\'data-ft\').catch(() => null);\n                        if (dataFt) {\n                            try {\n                                const ftData = JSON.parse(dataFt);\n                                if (ftData.fb_id) {\n                                    postID = ftData.fb_id;\n                                    console.log(`    Extracted Post ID (from data-ft.fb_id): ${postID}`);\n                                } else if (ftData.story_fbid) {\n                                    postID = ftData.story_fbid;\n                                    console.log(`    Extracted Post ID (from data-ft.story_fbid): ${postID}`);\n                                }\n                            } catch (e) { /* ignore JSON parse error */ }\n                        }\n                    }\n\n                    // Final fallback: use text hash (old method)\n                    if (!postID) {\n                        const textHash = postText.substring(0, 100).replace(/\\s/g, \'_\');\n                        postID = `hash_${textHash}`;\n                        console.warn(`    ⚠️ Using text hash for Post ID (could not find real FBID): ${postID}`);\n                    }\n                    // --- End Post ID extraction ---\n\n                    // const textHash = postText.substring(0, 100).replace(/\\s/g, \'_\');\n                    // const postID = `hash_${textHash}`;\n\n                    // **Check dedup in replies_log**\n                    const { data: alreadyReplied } = await supabase
                        .from('replies_log')
                        .select('*')
                        .eq('post_id', postID)
